...
class DashboardScreenState extends State<DashboardScreen> {
  ...
  Future<void> _showCameraOrGalleryDialog(BuildContext context) async {
    ...
    try {
      final hasActiveSubscription =
          await PaywallService.hasActiveSubscription();
      if (!hasActiveSubscription) {
        // Check if user has used their free scan
        final hasUsedFreeScan = await _checkIfUserHasUsedFreeScan();
        if (hasUsedFreeScan) {
          print(
            '🔒 No active subscription and free scan already used, showing paywall...',
          );

          // Check if user has used a referral code for metadata tracking
          final prefs = await SharedPreferences.getInstance();
          final hasUsedReferralCode =
              prefs.getBool('has_used_referral_code') ?? false;
          ...
          if (!purchased) {
            ...
            print(
              '💡 User closed Sale paywall, showing Offer paywall as fallback...',
            );
            await PaywallService.showPaywall(
              ...
              metadata: {
                'referral_code_used':
                    hasUsedReferralCode ? referralCode : 'none',
          ...
          print(
            '✅ No subscription but free scan available, allowing camera access...',
          );
        }
      }
    } catch (e) {
    ...
    try {
      ...
      if (hasUsedReferralCode) {
        ...
        if (usedReferralScans < referralFreeScans) {
          print(
            '🎁 User has ${referralFreeScans - usedReferralScans} referral free scans remaining',
          );
      ...
      if (user == null) {
        ...
        print(
          '🔍 Free scan check for non-authenticated user: hasUsed=$hasUsedFreeScan (${localMeals.length} local meals)',
        );
      ...
      print(
        '🔍 Free scan check for user ${user.uid}: hasUsed=$hasUsedFreeScan',
      );
      return hasUsedFreeScan;
    } catch (e) {
  ...
  void Function(List<Meal>) get updateMeals =>
      (meals) => setState(() => _meals = meals);
  ...
  Widget build(BuildContext context) {
    ...
    final double dailyCalories = _filteredMeals.fold(
      0.0,
      (sum, meal) => sum + meal.calories,
    );
    final double protein = _filteredMeals.fold(
      0.0,
      (sum, meal) => sum + (meal.macros['proteins'] ?? 0),
    );
    final double carbs = _filteredMeals.fold(
      0.0,
      (sum, meal) => sum + (meal.macros['carbs'] ?? 0),
    );
    final double fats = _filteredMeals.fold(
      0.0,
      (sum, meal) => sum + (meal.macros['fats'] ?? 0),
    );
    ...
    return Scaffold(
      ...
      appBar: SharedAppBar(title: 'navbar.dashboard'.tr()),
                  ...
                  try {
                    ...
                    if (user != null) {
                      ...
                      if (remainingMeals.isEmpty) {
                        print(
                          '🔄 All meals deleted - free scan is now available again',
                        );
                      }
                    }
                    await loadMealsFromFirebase();
